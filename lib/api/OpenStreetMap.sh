    ...
    if [[ ! -f "$map_pic" ]]; then
        for y in -1 0 1; do
            for x in -1 0 1; do
                tx=$((tile_x + x))
                ty=$((tile_y + y))
                curl -H "User-Agent: RainTracker.sh/0.0 - github.com/thatmfrick" -sf "https://tile.openstreetmap.org/$full_zoom/$tx/$ty.png" -o "$tmpdir/tile_${i}.png" &
                ((i++))
            done
        done
        wait
        ...
        magick "$map_pic" "$MAP_PIC" 2>/dev/null
